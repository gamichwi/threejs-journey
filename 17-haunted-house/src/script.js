...
const doorAmbientOcclusionTexture = textureLoader.load(
  './textures/door/ambienOcclusion.jpg'
);
...
//   './textures/roof/ambientOcclusion.jpeg'
// );
...
// roofAmbientOcclusionTexture.repeat.set(1, 4);
// roofNormalTexture.repeat.set(1, 4)
// roofRoughnessTexture.repeat.set(1, 4);
...
// gui.add(roofColorTexture.repeat, 'x', 0, 10).name('Repeat X');
// gui.add(roofColorTexture.repeat, 'y', 0, 10).name('Repeat Y');
// 6
...
// roof.material.map.rotation = Math.PI / 4;
house.add(roof);
...
door.position.z = 2 + 0.01; //removes Z fighting.
door.position.y = 1;
house.add(door);

// Bushes - placed manually
...
const graveMaterial = new THREE.MeshStandardMaterial({
  ...
  roughness: gravesRoughnessTexture,
});
...
for (let i = 0; i < 50; i++) {
  ...
  grave.rotation.x = (Math.random() - 0.5) / 8;
